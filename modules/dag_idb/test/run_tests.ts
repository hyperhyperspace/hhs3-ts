...
async function main() {
    ...
    allTests.set(idbDagDbSuite.title, idbDagDbSuite.tests);
    allTests.set(crossTabSuite.title, crossTabSuite.tests);

    console.log('Running tests for HHSv3 IndexedDB DAG module' + (filters.length > 0 ? ' (applying filter: ' + filters.toString() + ')' : '') + '\n');
