...
- **project** (`project_parity.ts`, generator `project_generate.ts`): for every extending checkpoint pair in a random schema+row history, the `full` re-projection, the `incremental` projection, and the live `rdb` projection must all agree.
- **ingest** (`ingest_parity.ts`, generator `ingest_generate.ts`): the `optimized` adapter config (`updateMerge` + `fkBundling`) and a `naive` config must produce equivalent live views for the same captured outbox.
...
The fuzzer size is set by a named profile in `test/planner_parity/profiles.ts` (`seeds / ops / maxPairs / ingestBatches / ingestChanges`):
