...
export async function createRandomBranchingDags(constrs: [()=>Dag|Promise<Dag>, ()=>Dag|Promise<Dag>], seed: number, size: number, options?: { progressBar?: boolean, instanceCount?: number }): Promise<{ dags: Array<[Dag, Dag]>, branches: Array<[Position, Position]>}> {
    ...
    for (let i=0; i<(options?.instanceCount || 20); i++) {
        ...
        const d1 = await constrs[0]();
        const d2 = await constrs[1]();
...
export async function createRandomDisconnectedDags(constrs: [()=>Dag|Promise<Dag>, ()=>Dag|Promise<Dag>], seed: number, size: number, options?: {connectedBranches?: boolean, progressBar?: boolean}): Promise<{ dags: Array<[Dag, Dag]>, branches: Array<[Position, Position]>}> {
    const dags: Array<[Dag, Dag]> = [];
    const branches: Array<[Position, Position]> = [];
...
export async function createRandomDags(constrs: [()=>Dag|Promise<Dag>, ()=>Dag|Promise<Dag>], seed: number, size: number, options?: { progressBar?: boolean, addMeta?: boolean }): Promise<{ dags: Array<[Dag, Dag]>, branches: Array<[Position, Position]>}> {
    const dags: Array<[Dag, Dag]> = [];
    const branches: Array<[Position, Position]> = [];
