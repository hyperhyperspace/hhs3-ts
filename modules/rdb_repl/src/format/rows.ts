...
export type FormatRowsOptions = { ctx?: HashDisplayContext; structuralColumns?: Set<string> };
...
export function formatRows(
    ...
    const rendered = rows.map((row) => cols.map((column) =>
        renderCell(row[column], options?.ctx, structural.has(column) ? 'hash' : 'cell')));
...
export function formatRowsVertical(
    ...
    return rows.map((row, index) => [
        `*** row ${index + 1} ***`,
        ...cols.map((column) =>
            `${column}: ${renderCell(row[column], options?.ctx, structural.has(column) ? 'hash' : 'cell')}`),
    ].join('\n')).join('\n\n');
}

function renderCell(value: unknown, ctx?: HashDisplayContext, role: 'hash' | 'cell' = 'cell'): string {
