export function formatRows(rows: Record<string, unknown>[]): string {
    ...
    const widths = columns.map((column, i) => Math.max(column.length, ...rendered.map((row) => row[i].length)));
...
export function formatRowsVertical(rows: Record<string, unknown>[]): string {
    if (rows.length === 0) return '(0 rows)';
    const columns = [...new Set(rows.flatMap((row) => Object.keys(row)))];
    return rows
        .map((row, index) => {
            const header = `*** row ${index + 1} ***`;
            const lines = columns.map((column) => `${column}: ${renderCell(row[column])}`);
