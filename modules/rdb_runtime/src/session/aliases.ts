...
async function candidatesForScope(scope: AliasScope, session: AliasSession): Promise<B64Hash[]> {
    if (scope === 'key') {
        if (session.keyVault === undefined) return [];
        return session.keyVault.list().map((k) => k.keyId);
    }
...
export async function collectVersionOpHashes(roots: import("../workspace/root_index.js").RootIndex): Promise<B64Hash[]> {
...
export async function aliasLabel(
    ...
    if (scope === 'key') {
        return session.keyVault?.list().find((k) => k.keyId === hash)?.label;
    }
    if (scope === 'version') return rootNameForVersionHash(hash, session.workspace.roots);
    return session.workspace.roots.get(hash)?.name;
}

export async function rootNameForVersionHash(
    hash: B64Hash,
    roots: import("../workspace/root_index.js").RootIndex,
): Promise<string | undefined> {
    for (const root of roots.list()) {
        if (root.object === undefined) continue;
        const dag = await root.object.getScopedDag();
