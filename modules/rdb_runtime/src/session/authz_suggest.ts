...
export function labelForKeyId(session: RdbSession, keyId: KeyId): string {
    const record = session.keyVault?.list().find((key) => key.keyId === keyId);
...
function toCandidate(session: RdbSession, record: KeyRecord): AuthorCandidate {
    return {
        keyId: record.keyId,
        label: record.label,
...
function candidateFromIdentity(session: RdbSession, identity: OwnIdentity): AuthorCandidate {
    const record = session.keyVault?.list().find((key) => key.keyId === identity.keyId);
    return {
...
export async function scanKeystore(
    ...
    for (const record of session.keyVault.list()) {
