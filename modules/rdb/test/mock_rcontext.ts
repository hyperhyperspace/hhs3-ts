...
export type MockRContextOptions = {
    // Injected mesh: makes getMesh return it instead of throwing (the RDb
    // fan-out tests inject a stub mesh that counts createSwarm calls).
    mesh?: any;
    // Injected fetchObject: bootstrap for members absent from the replica.
    fetchObject?: (id: B64Hash, opts?: { meshLabel?: string; backendLabel?: string; timeoutMs?: number }) => Promise<RObject>;
};

export function createMockRContext(
    config: RObjectConfig = { selfValidate: true },
    opts: MockRContextOptions = {},
): RContext {
